...
public interface ResourceRepository extends Neo4jRepository<Resource, Long> {
    ...
    @Query("Match (role:Role {roleId:{roleId}})<-[roleRel:HAS_ROLE]-(rg:RoleGroup)<-[groupRel:HAS_GROUP]-(org:Organization)<-[member:BELONGS_TO {status:\"active\"}]-(p:Person {personId: {personId}})-[personRel:IS_A]->(role:Role {roleId:{roleId}})-[rel:HAS_ACCESS]->(res:Resource {route:{route}, method:{method}}) return count(role) > 0 as c")
